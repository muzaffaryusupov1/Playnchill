    ...
    <div className='my-6'>
      <div className='container'>
        <div>
          ...
          <div className='flex flex-wrap gap-8 max-[812px]:gap-y-20 max-[572px]:gap-y-10 max-[572px]:gap-1 max-[430px]:items-center'>
            {
              alist.map(item => (
                <div className='py-14 p-6 w-[370px] flex flex-col items-center max-[812px]:py-4 max-[812px]:p-4 max-[812px]:w-auto max-[793px]:p-0' key={item.id}>
                  <div className='flex items-center gap-4 mb-7 max-[793px]:mb-4 max-[793px]:gap-2'>
                    <div className='w-[70px] h-[70px] max-[812px]:w-60px max-[812px]:h-60px max-[793px]:w-[50px] max-[793px]:h-[50px]'>
                      <img src={item.image} alt={item.title} className='w-full h-full object-contain' />
                    </div>
                    <p className='w-[120px] font-bold text-xl text-white] max-[812px]:w-[90px] max-[812px]:text-base max-[793px]:text-sm'>{item.title}</p>
                  </div>
                  <div>
                    <p className='font-normal text-base text-white opacity-60 text-center max-[812px]:text-sm max-[812px]:w-[300px] max-[793px]:w-[250px] max-[793px]:text-sm max-[572px]:text-xs'>{item.subtitle}</p>
                  </div>
                </div>
              ))
            }
